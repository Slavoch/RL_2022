...
class AnimatorInvertedPendulum(Animator):
    def __init__(self, objects=[], pars=[]):
        ...
        self.policy_line_handles_pack = self.axs_action_params.plot(
            [0],
            [self.scenario.actor.model.weights],
            label=[r"$\theta_1$", r"$\theta_2$", r"$\theta_3$"],
        )
        plt.legend()
        ###################################################################
    ...
    def update_iteration_playback(self):
        ...
        for i, handle in enumerate(self.policy_line_handles_pack):
            update_line(
                ...
                self.thetas[self.current_step][i],
            )
