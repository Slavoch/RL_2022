...
class Pipeline2Tank(AbstractPipeline):
    def initialize_system(self):
        self.my_system = systems.Sys2Tank(
            sys_type="diff_eqn",
    ...
    def initialize_controller(self):
        self.controller = controllers.RLController(
            ...
            compute_state_dynamics=self.my_system._state_dyn,
        ...
        for k in range(0, self.Nruns):
            ...
            if self.is_log:
                ...
                with open(self.datafiles[k], "w", newline="") as outfile:
                    ...
                    writer.writerow(["System", self.my_system.name])
                    ...
                    writer.writerow(
                        ["t [s]", "h1", "h2", "p", "running_objective", "accum_obj"]
                    )
    ...
    def main_loop_visual(self):
        ...
        my_animator = animators.Animator2Tank(
            objects=(
                self.my_simulator,
                self.my_system,
                [],
    ...
    def main_loop_raw(self):
        ...
        while True:
            ...
            time, state, observation, state_full = self.my_simulator.get_sim_step_data()
            ...
            if not self.no_print:
                self.my_logger.print_sim_step(
                    time, h1, h2, p, running_objective, accum_obj
                )

            if self.is_log:
                self.my_logger.log_data_row(
                    datafile, time, h1, h2, p, running_objective, accum_obj
                )
            ...
            if time >= self.time_final:
                ...
                self.my_simulator.time = self.time_start
