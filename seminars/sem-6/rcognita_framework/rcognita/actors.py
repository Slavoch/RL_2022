...
class Actor:
    ...
    def __init__(
        ...
        if len(action_init) == 0:
            self.model.cache.weights = (self.action_min + self.action_max) / 2
            self.action_sequence_init = rc.rep_mat(
                self.model.cache.weights, 1, self.prediction_horizon + 1
            )
            self.action_init = self.model.cache.weights
        else:
            self.action_init = action_init
            self.model.cache.weights = action_init
            self.action_sequence_init = rc.rep_mat(
        ...
        self.action = self.model.cache.weights
    ...
    def update(self, observation, constraint_functions=(), time=None):
        ...
        # self.g_actor_values.append([g1(action_sequence), time])

        # row_header = ["g1"]
        # row_data = [g1(action_sequence)]
        # row_format = "8.3f"
        # table = tabulate(
        #     [row_header, row_data],
        #     floatfmt=row_format,
        ...
        # self.model.cache.weights = self.action
...
class ActorProbabilisticEpisodic(Actor):
    ...
    def reset(self):
        self.gradients = []
        self.model.cache.weights = self.action_init
